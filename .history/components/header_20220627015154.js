              ...
              <ul className="flex justify-between w-full text-xl">
                ...
                <li className="flex items-center cursor-pointer text-gray-500 font-semibold justify-between w-[20%] transition-all ease-in-out duration-300 relative dark:text-white">
                  <Menu>
                    {({open}) => (
                      <>
                        <Menu.Button className="flex items-center justify-center gap-4" onClick={() => setEnabled(!enabled)}>Features <Icon icon="akar-icons:chevron-down" className={`${enabled ? 'rotate-0' : 'rotate-180 transition-all duration-150 ease-in-out'}`}/>
                        </Menu.Button>
                        {open && (
                          <>
