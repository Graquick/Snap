      ...
      <header className="flex w-full mx-auto justify-between sticky top-0 z-10 bg-neutral-100 h-28 items-center lg:h-24 lg:px-4 dark:bg-neutral-800">
        <div className="flex w-[850px] justify-between items-center 2xl:w-[750px] xl:w-[600px]">
            ...
            <div className="w-[600px] 2xl:w-[500px] xl:w-[360px] lg:hidden ">
              <ul className="flex justify-between w-full text-xl 2xl:text-[1.125rem] xl:text-[.75rem]">
                    ...
                    {({open}) => (
                      <>
                        <Switch checked={enabled} onChange={setEnabled} className="flex items-center justify-center gap-2 xl:gap-1">
                          Features <Icon icon="akar-icons:chevron-down"  className={`${enabled ? 'rotate-180' : 'rotate-0'} transition-all duration-150 ease-in-out xl:text-xs`}/>
                        </Switch>
                        <AnimatePresence ExitBeforeEnter>
                            {enabled && (
                              <>
                                <Menu.Items static 
                                  className="absolute left-0 z-10 flex flex-col justify-center bg-white text-black h-fit focus:outline-none w-44 top-12 dark:border-neutral-500 2xl:w-36 2xl:gap-2 xl:w-24"
                                  as={motion.div}
                                  initial={{ y: 100}}
                                  animate={{ y: 0}}
                      ...
                      {({open}) => (
                        <>
                          <Switch checked={enabledTwo} onChange={setEnabledTwo} className="flex items-center justify-center gap-2">
                            Company <Icon icon="akar-icons:chevron-down"  className={`${enabledTwo ? 'rotate-180' : 'rotate-0'} transition-all duration-150 ease-in-out xl:text-xs`}/>
                          </Switch>
                          <AnimatePresence exitBeforeEnter>
                              {enabledTwo && (
                                  <>
                ...
                <li className="flex items-center justify-center font-semibold text-gray-500 transition-all duration-300 ease-in-out cursor-pointer w-fit dark:text-white 2xl:relative 2xl:left-4">Careers</li>
