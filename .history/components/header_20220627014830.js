              ...
              <ul className="flex justify-between w-full text-xl">
                ...
                <li className="flex items-center cursor-pointer text-gray-500 font-semibold justify-between w-[20%] transition-all ease-in-out duration-300 relative dark:text-white">
                  <Menu>
                    {({open}) => (
                      <>
                        <Menu.Button className="flex items-center justify-center gap-4" onClick={() => setEnabled(!enabled)}>Features <Icon icon="akar-icons:chevron-down" />
                        </Menu.Button>
                        {open && (
                          <>
                            <Menu.Items static className={`${enabled ? '' :' dark:border-neutral-500 border rounded-md'}`}
                              style={{ position: "absolute", top: "3rem", left: 0, zIndex: 10, width: "11rem", height: "14rem", background: "white", outline: "none"  }}
