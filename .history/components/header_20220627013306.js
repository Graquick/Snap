              ...
              <ul className="flex justify-between w-full text-xl">
                ...
                <li className="flex items-center cursor-pointer text-gray-500 font-semibold justify-between w-[20%] transition-all ease-in-out duration-300 relative dark:text-white">
                  <Menu>
                    {({open}) => (
                      <>
                        <Menu.Button className="flex items-center justify-center gap-4" onClick={() => !}>Features     <Icon icon="akar-icons:chevron-down" />
                        </Menu.Button>
                        {open && (
                          <>
                            <Menu.Items static className="absolute left-0 z-10 h-56 bg-white border rounded-md focus:outline-none w-44 top-12 dark:border-neutral-500"
                              as={motion.div}
                              initial={{ x: -100, y: -100}}
                              animate={{ x: 0, y: 0}}
                              transition={spring}
