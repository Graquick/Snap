                  ...
                  <Menu>
                    {({open}) => (
                      <>
                        ...
                        {open && (
                          <>
                            <Menu.Items className="absolute left-0 w-40 bg-white border rounded-md top-12 whitespace-nowrap h-44">
                              <Menu.Item className="flex items-center justify-center gap-2 px-5 py-3">
                                {({active}) => (
                                  <MyLink href="#" className={`${active ? 'bg-blue-400 text-white' : 'bg-green-600 text-black'}`}><Icon icon="uil:clipboard-notes" />Todo List</MyLink>
                                )}
                              </Menu.Item>
                              
                              <Menu.Item className="flex items-center justify-center gap-2 px-5 py-3">
                                {({active}) => (
                                  <MyLink href="#" className={`${active ? 'bg-blue-400 text-black' : 'bg-red-600 text-red-400'}`}><Icon icon="uil:calendar-alt" />Calendar</MyLink>
                                )}
                              </Menu.Item>
                            </Menu.Items> 
                          </>
