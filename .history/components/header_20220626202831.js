  ...
  return (
    <>
      <header className="flex max-w-[75%] m-auto justify-between sticky top-0 h-36 items-center">
        <div className="flex w-[45%] justify-between items-center" onClick={() => setEnabled(!enabled)}>
            ...
            <div className="w-[70%]">
              ...
              <li className="flex items-center cursor-pointer text-gray-500 font-semibold justify-between w-[20%] hover:text-black transition-all ease-in-out duration-300 relative">
                  <Menu>
                    Features
                    <Menu.Button>
                        <motion.div transition={spring} onClick={() => setEnabled(!enabled)}><Icon icon="akar-icons:chevron-down" className={!enabled ? "rotate-0 transition-all 300ms ease" : "rotate-180 transition-all 300ms ease"}/></motion.div>
                    </Menu.Button>
