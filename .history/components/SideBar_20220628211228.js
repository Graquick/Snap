            ...
            >
                <ul className="absolute top-24 left-0 flex flex-col gap-8 items-start text-left w-[250px] h-[650px] text-xl pl-12">
                            ...
                            <Menu>
                                {({open}) => (
                                    <>
                                    <Switch checked={enabledTwo} onChange={setEnabledTwo} as={motion.div} whileHover={{ scale: 1.1, x: -10, y: -5 }} className={`${enabledTwo ? 'text-blue-400 dark:text-purple-400' : 'text-black dark:text-white'} flex items-center justify-start gap-4`}>
